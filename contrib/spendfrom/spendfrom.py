...
def determine_db_dir():
    """Return the default location of the bitcredit data directory"""
...
def connect_JSON(config):
    """Connect to a bitcredit JSON-RPC server"""
...
def list_available(bitcreditd):
    ...
    for output in unspent:
        ...
        # This code only deals with ordinary pay-to-bitcredit-address
...
def create_tx(bitcreditd, fromaddresses, toaddress, amount, fee):
    ...
    if total_available < needed:
        sys.stderr.write("Error, only %f BCR available, need %f\n"%(total_available, needed));
        sys.exit(1)
...
def main():
    ...
    parser.add_option("--from", dest="fromaddresses", default=None,
                      help="addresses to get bitcredits from")
