...
The RPM spec file provided here is for Bitcredit-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcredit.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcredit git repository](https://github.com/bitcredit/bitcredit) but are not part of
...
Currently only `Source10` is used, for the example `bitcredit.conf` file.
...
in the bitcredit git repository. At present only an SVG version of the Bitcredit
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcredit
...
needed when 0.12.1 is released, a proper fix is already in the Bitcredit git
master branch.
...
idea to build Bitcredit Core against a static version of that library compiled
...
The `bitcredit.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcredit-server` package. At this point, most RPM based
...
The `bitcredit-server` package should have SELinux support. How to properly do
...
Bitcredit-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcredit/). He has been
packaging Bitcredit for Fedora at least since 2012.
...
4. I renamed the `bitcredit` package that contains the Qt GUI to `bitcredit-core` as
that appears to be how the general population refers to it, in contrast to
`bitcredit-xt` or `bitcredit-classic`. I wanted to make sure the general population
