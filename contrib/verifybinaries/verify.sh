...
###   This script attempts to download the signature file SHA256SUMS.asc from bitcredit.org
...
WORKINGDIR="/tmp/bitcredit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://bitcredit.org/bin/"
VERSIONPREFIX="bitcredit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcredit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcredit Core binary release signing key installed?"
   fi
