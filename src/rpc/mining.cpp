...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or " + std::string(BITCREDIT_CONF_FILENAME) + " setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    GenerateBitcredits(fGenerate, nGenProcLimit, Params());
...
UniValue generatetoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. numblocks    (numeric, required) How many blocks are generated immediately.\n"
            "2. address    (string, required) The address to send the newly generated bitcredit to.\n"
...
// NOTE: Unlike wallet RPC (which use BCR values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue estimatesmartfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in BCR)\n"
