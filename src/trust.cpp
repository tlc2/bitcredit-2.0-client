...
std::map<std::string,int> banlist(){
std::map<std::string,int> users;
	fstream myfile ((GetDataDir()/ "loandata/verifieddata.dat").string().c_str());
	char * pEnd;
	std::string line;
	if (myfile.is_open()){
		while ( myfile.good() ){
			...
			std::vector<std::string> strs;
			boost::split(strs, line, boost::is_any_of(","));
			users[strs[0]]=strtoll(strs[1].c_str(),&pEnd,10);
		}
		myfile.close();
	}
	return users;
...
std::map<std::string,double> creditratings(){
...
std::map<std::string,int> verifieddata(){
std::map<std::string,int> users;
	fstream myfile ((GetDataDir()/ "loandata/verifieddata.dat").string().c_str());
	char * pEnd;
	std::string line;
