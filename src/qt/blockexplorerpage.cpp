...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcreditUnits::formatWithUnit(BitcreditUnits::BCR, nValue);
...
std::string TxToString(uint256 BlockHash, const CTransaction& tx)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        {
            itostr(i),
            (HashNext == 0)? (fAddrIndex? _("no") : _("unknown")) : "<span class=\"mono\">" + makeHRef(HashNext.GetHex()) + ":"  + "</span>",
...
std::string AddressToString(const CBitcreditAddress& Address)
{
    ...
    AddressScript= GetScriptForDestination(dest);
	uint160 addrid = 0;
    int64_t Sum = 0;
    ...
    {
        std::vector<CExtDiskTxPos> Txs;
        pblocktree->ReadAddrIndex(addrid, Txs);
        BOOST_FOREACH (const CDiskTxPos& pos, Txs)
        {
            CTransaction tx;
            CBlock block;
            uint256 hash, hashBlock;
            ReadTransaction(tx, pos, hashBlock);
...
void BlockExplorerPage::keyPressEvent(QKeyEvent *event)
{
...
void BlockExplorerPage::showEvent(QShowEvent*)
{
    if (m_NeverShown)
    {
        m_NeverShown = false;

        setBlock(chainActive.Tip());
...
bool BlockExplorerPage::switchTo(const QString& query)
{
    bool IsOk;
    int AsInt = query.toInt(&IsOk);
    if (IsOk && AsInt >= 0 && AsInt <= chainActive.Tip()->nHeight)
    {
        CBlockIndex* pIndex = chainActive[AsInt];
        if (pIndex)
        {
            setBlock(pIndex);
            return true;
...
void BlockExplorerPage::goTo(const QString& query)
{
    if (switchTo(query))
    {
...
void BlockExplorerPage::onSearch()
{
    goTo(ui->searchBox->text());
}

void BlockExplorerPage::setBlock(CBlockIndex* pBlock)
{
    setContent(BlockToString(pBlock));
}

void BlockExplorerPage::setContent(const std::string& Content)
{
...
void BlockExplorerPage::forward()
{
    int NewIndex = m_HistoryIndex + 1;
    if (0 <= NewIndex && NewIndex < m_History.size())
    {
...
void BlockExplorerPage::updateNavButtons()
{
