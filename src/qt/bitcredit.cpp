...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcredit_de.qm (shortcut "de" needs to be defined in bitcredit.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcredit_de_DE.qm (shortcut "de_DE" needs to be defined in bitcredit.qrc)
...
void BitcreditCore::handleRunawayException(const std::exception *e)
{
    ...
    returnValue(0)
{
    ...
    // This must be done inside the BitcreditApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = GetArg("-uiplatform", BitcreditGUI::DEFAULT_UIPLATFORM);
...
void BitcreditApplication::createOptionsModel(bool resetSettings)
{
...
{
    window = new BitcreditGUI(platformStyle, networkStyle, 0);
...
void BitcreditApplication::parameterSetup()
{
    InitLogging();
