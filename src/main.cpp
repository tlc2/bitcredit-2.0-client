                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s BCR additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("bitcredit-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
	...
	int64_t devfund = (GetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus()))* (0.2);
    ...
    if (chainActive.Tip()->nHeight > 1 && block.vtx[0].GetValueOut() > blockReward)
...
bool InitBlockIndex(const CChainParams& chainparams)
{
	...
	PUBKEY_SCRIPT << OP_DUP << OP_HASH160 << ParseHex(DEV_ADDRESS) << OP_EQUALVERIFY << OP_CHECKSIG;
