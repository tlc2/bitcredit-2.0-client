Bitcredit Core version 0.11.0 is now available from:

  <https://bitcredit.org/bin/bitcredit-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BCR/kB fee to be
...
For example, add the following to `bitcredit.conf`:
...
related to the blockchain in the bitcredit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Bitcredit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Bitcredit Core will 
...
For an example script see [bitcredit-submittx](https://github.com/laanwj/bitcredit-submittx).
...
- #5334 `cf87536` libbitcreditconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new bitcredit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only BitcreditAmountField
- #5753 `0f44672` Add bitcredit logo to about screen
...
- #5809 `46bfbe7` Add bitcredit-cli man page
...
- #5849 `41259ca` contrib/init/bitcreditd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Bitcredit-Qt.app to Bitcredit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitcredit-tx)
