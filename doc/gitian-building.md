...
Clone the git repositories for bitcredit and Gitian.
...
in the bitcredit repository under 'Fetch and build inputs' to install sources which require
...
To build Bitcredit (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitcredit repository.
...
and the bitcredit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcredit/contrib/gitian-descriptors/*|sort|uniq )
...
BCRPATH=/some/root/path/bitcredit.git
SIGPATH=/some/root/path/bitcredit-detached-sigs.git

./bin/gbuild --url bitcredit=${BCRPATH},signature=${SIGPATH} ../bitcredit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcredit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcredit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcredit-osx-build.assert
```
...
[bitcredit-core/gitian.sigs](https://github.com/bitcredit-core/gitian.sigs/) repository, or if that's not possible create a pull
